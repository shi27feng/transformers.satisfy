...
class BipartiteData(Data):
    def __init__(self, pos_adj, neg_adj, xv, xc):
        ...
        self.edge_index_var_pp = self.edge_index_var_pn = self.edge_index_var_np = self.edge_index_var_nn = None
        self.edge_index_cls_pp = self.edge_index_cls_pn = self.edge_index_cls_np = self.edge_index_cls_nn = None
    ...
    def _meta_paths_(self, adj_pos, adj_neg):
        ...
        val_pos = torch.ones_like(adj_pos[0], dtype=torch.float)
        val_neg = torch.ones_like(adj_neg[0], dtype=torch.float)
        ...
        self.edge_index_var_pp, self.edge_index_var_pn, self.edge_index_var_np, self.edge_index_var_nn = \
            self._cross_product(adj_pos_t, adj_pos, adj_neg_t, adj_neg, val_pos, val_neg, n, m)

    def _put_back_cpu(self):
        self.edge_index_var_pp = self.edge_index_var_pp.to("cpu")
        self.edge_index_var_pn = self.edge_index_var_pn.to("cpu")
        self.edge_index_var_np = self.edge_index_var_np.to("cpu")
        self.edge_index_var_nn = self.edge_index_var_nn.to("cpu")
    ...
    def __inc__(self, key, value):
        ...
        if key in ['edge_index_pos', 'edge_index_neg']:
            return torch.tensor([[self.xc.size(0)], [self.xv.size(0)]])
        elif key in ['edge_index_var_pp', 'edge_index_var_pn', 'edge_index_var_np', 'edge_index_var_nn']:
            return torch.tensor([[self.xv.size(0)], [self.xv.size(0)]])
...
class CNFParser:
    def __init__(self):
        ...
        self.edge_index_pos = [[], []]  # [clauses, variables]
        self.edge_index_neg = [[], []]
        self.satisfied = True
        self.text = None
